...
import { EffectComposer, Bloom, Noise, SSAO, SMAA } from '@react-three/postprocessing'
...
import ForestA from "./Forest-audio.mp3";
import ThunderA from "./Thunder-audio.mp3";
import CanyonA from "./Canyon.mp3";
import Button from '@mui/material/Button';
import { Lightmap } from './Lightmap'
import { Particles } from './Particles'
import setInitialPositions from "./set-initial-positions";
import { setSyntheticTrailingComments } from "typescript";
//import MD_Roof_Black_basecolor from './Textures/MD_Roof_Black_basecolor.jpg'
//import MD_Roof_basecolor from '/Textures/MD_Roof_basecolor.jpg'
//import MD_Roof_Gray_basecolor from '/Textures/MD_Roof_Gray_basecolor.jpg'
//import MD_Roof_height from '/Textures/MD_Roof_height.jpg'
//const [Roof_1, Roof_2, Roof_3,Roof_H] = useLoader(THREE.TextureLoader, [MD_Roof_basecolor, MD_Roof_Black_basecolor, MD_Roof_Gray_basecolor ,MD_Roof_height ,])
const main = {
  boxSizing: "inherit",
  display: "flex"
};
...
export default function App() {
 ...
 const D =150
  ...
  return (
    <div style={main} >
      <div style={styleMenu} >
        <Manu setInside={setInside} Inside={Inside} setClosedPro={setClosedPro} ClosedPro={ClosedPro} setForest={setForest} setDesert={setDesert} setDry={setDry} setWind={setWind} setStorm={setStorm}   setColor={setColor} setRColor={setRColor} setHColor={setHColor} setWColor={setWColor} setWPColor={setWPColor} setRTColor={setRTColor} roofColor={roofColor} houseColor={houseColor} windowsColor={windowsColor} tubeColor={tubeColor} winProColor={winProColor}/>
      </div>
      <div style={styleMainMenu} >
        <MainMenu Day={Day} setDay={setDay}/>
      </div> 
      
    <Canvas sRGB colorManagement shadows dpr={[2, 3]} gl={{  preserveDrawingBuffer: true ,depth:true,  powerPreference: "low-power"}} style={styleCanvas} onCreated={({ gl }) => {
        ...
        <Lightmap position={[50, 150, 50]} {...propser} >
           <HouseModel_inside />
            ...
            {InsideRooms.map((room)=>
                ...
                fullscreen 
                >
                     <Button size="humongous" sx={{fontSize:3 ,fontWeight: 600}} onClick={(e) => setPostList(room.pos)} variant="contained"><h5>{room.name}</h5></Button>
                </Html>
          )}
           </Lightmap>
        </Suspense>
        <PerspectiveCamera position={150, Math.PI * 8, 12} fov={250} zoom={-1} />
        <OrbitControls target={[PostList.value1, PostList.value2, PostList.value3]} rotateSpeed={Math.PI * 0.5} position={0,20,1} minPolarAngle={Math.PI / 3} minDistance={Math.PI  * 0.01}  enablePan={false} maxDistance={Math.PI * 0.01} maxPolarAngle={1.7}  />
        </>
     ...
     <Suspense fallback={null}>
      ...
      shadow-mapSize-width={4096}
       ...
       />
        ...
        fullscreen 
        >
        <Button onClick={(e) => setInside(true)} variant="contained">inside</Button>
      </Html>
          <HouseModel Storm={Storm} ClosedPro={ClosedPro} Forest={Forest} Desert={Desert} MeshStaticData={MeshStaticData} DistData={DistData} MeshData={MeshData}  />
        </Suspense>
        <OrbitControls makeDefault rotateSpeed={Math.PI * 0.5} position={0,100,3} minPolarAngle={Math.PI / 4} minDistance={Math.PI  * DesertCameraMin}  enablePan={false} maxDistance={Math.PI * DesertCameraMax} maxPolarAngle={DesertCAngle} />
      {Storm? <>
        <Rains {...proped}  />
...
function MainMenu({setDay,Day }) {
  ...
  return (
    <Paper sx={{ width: 320 }}>
      <MenuList dense>
      <MenuItem onClick={(e) => setDay(true)} sx={{bgcolor:colorme ,marginTop: "1px"}} > Day</MenuItem>
      <MenuItem onClick={(e) => setDay(false)} sx={{bgcolor:colormeN ,marginTop: "1px"}} > Night</MenuItem>
       </MenuList>
      </Paper>
  )
}
...
function Manu({setInside, Inside,setDesert ,setForest,ClosedPro,setClosedPro, setStorm,setDry, setWind, setRColor,setHColor, setWColor,setWPColor,setRTColor,roofColor,houseColor ,windowsColor ,tubeColor ,winProColor}) {
  ...
  return (
    < >
     <Paper sx={{ width: 320 }}>
      ...
      <MenuList dense>
        {Inside?<></>:<div>
          <ListItemText sx={{bgcolor:"#d6623540 ",marginTop: "1px"}} inset>Kolor dachu</ListItemText>
         <M.Stack  sx={{bgcolor:"#ffc7b11a ",marginTop: "1px"}}  direction="row"
  divider={<Divider orientation="vertical" flexItem />} spacing={0.8}>
       ...
       <ListItemText sx={{bgcolor:"#d6623540 ",marginTop: "1px"}} inset>Kolor elewacji</ListItemText>
         <M.Stack sx={{bgcolor:"#ffc7b11a ",marginTop: "1px"}}  direction="row"
  divider={<Divider orientation="vertical" flexItem />} spacing={0.8}>
       ...
       <ListItemText  sx={{bgcolor:"#d6623540 ",marginTop: "1px"}}  inset>Kolor okien</ListItemText>
         <M.Stack sx={{bgcolor:"#ffc7b11a ",marginTop: "1px"}}  direction="row"
  divider={<Divider orientation="vertical" flexItem />} spacing={0.8}>
       ...
       <ListItemText  sx={{bgcolor:"#d6623540 ",marginTop: "1px"}}  inset>Kolor rynien</ListItemText>
         <M.Stack sx={{bgcolor:"#ffc7b11a ",marginTop: "1px"}}  direction="row"
  divider={<Divider orientation="vertical" flexItem />} spacing={0.8}>
       ...
       <ListItemText  sx={{bgcolor:"#d6623540 ",marginTop: "1px"}}  inset>Kolor rolet</ListItemText>
         <M.Stack sx={{bgcolor:"#ffc7b11a ",marginTop: "1px"}}  direction="row"
  divider={<Divider orientation="vertical" flexItem />} spacing={0.8}>
       ...
       <ListItemText></ListItemText>
        ...
        <ListItemText sx={{bgcolor:"#d6623540 ",marginTop: "1px"}}  inset >Environment</ListItemText>
        <MenuItem>
        <ListItemText onClick={(e) => {setForest(false) ,setDesert(true) }} >Desert</ListItemText>
        </MenuItem>
         <Divider />
        <MenuItem>
          <ListItemText onClick={(e) => {setDesert(false) ,setForest(true) }} >Forest</ListItemText>
        </MenuItem>
         <Divider />

         <ListItemText sx={{bgcolor:"#d6623540 ",marginTop: "1px"}}  inset >Wheaher</ListItemText>
         <MenuItem>
        <ListItemText onClick={(e) => {setStorm(true) ,setDry(false) ,setWind(false)}} >Storm</ListItemText>
        </MenuItem>
        <MenuItem>
          <ListItemText onClick={(e) => {setStorm(false) ,setDry(true) ,setWind(false)}}>Normal</ListItemText>
        </MenuItem>
        <MenuItem>
          <ListItemText onClick={(e) => {setStorm(false) ,setDry(false) ,setWind(true)}}>Wind</ListItemText>
        </MenuItem>
         <Divider />
         <ListItemText sx={{bgcolor:"#d6623540 ",marginTop: "1px"}}  inset >view</ListItemText>
        <MenuItem>
        <ListItemText  onClick={(e) => {setInside(true)}}>Inside</ListItemText>
        </MenuItem>
        <MenuItem>
        <ListItemText  onClick={(e) => {setInside(false)} } >Outside</ListItemText>
        </MenuItem>
        <Divider />
        {ClosedPro?
        <MenuItem>
        <ListItemText onClick={(e) => {setClosedPro(false)}}>Closed</ListItemText>
        </MenuItem>
        :
        <MenuItem>
        <ListItemText onClick={(e) => {setClosedPro(true)}}>Close Window</ListItemText>
        </MenuItem>
        }
        <MenuItem>
